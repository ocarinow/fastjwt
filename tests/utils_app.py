...
def init_app(config: Optional[FJWTConfig] = None) -> tuple[FastAPI, FastJWT]:
    """Initialize FastAPI app and FastJWT instance."""

    app = FastAPI()
    security = FastJWT(config=(config if config else FJWTConfig()))

    return app, security


def create_securities(security: FastJWT) -> SecuritiesTuple:
    """Create a named tuple with all FastJWT dependencies."""
...
def create_get_token_routes(app: FastAPI, security: FastJWT) -> None:
...
def create_token_routes(app: FastAPI, security: FastJWT) -> None:
    """Create routes to get tokens from FastJWT instance."""

    @app.get("/token/access")
    def _access_token_route(fjwt: FastJWTDeps = security.DEPENDENCY):
        token = fjwt.create_access_token(uid="test", fresh=False)
        fjwt.set_access_cookies(token)
        return {"token": token}

    @app.get("/token/fresh")
    def _fresh_token_route(fjwt: FastJWTDeps = security.DEPENDENCY):
        token = fjwt.create_access_token(uid="test", fresh=True)
        fjwt.set_access_cookies(token)
        return {"token": token}

    @app.get("/token/refresh")
    def _refresh_token_route(fjwt: FastJWTDeps = security.DEPENDENCY):
        token = fjwt.create_refresh_token(uid="test")
        fjwt.set_refresh_cookies(token)
        return {"token": token}


def create_secure_routes(app: FastAPI, securities: SecuritiesTuple) -> None:
    """Create routes to test FastJWT dependencies."""
...
def create_protected_routes(app: FastAPI, security: FastJWT) -> None:
    """Create routes to test FastJWT dependencies."""
...
def create_blocklist_routes(app: FastAPI, security: FastJWT) -> None:
    """Create routes to test FastJWT dependencies."""
...
def create_subject_routes(app: FastAPI, security: FastJWT) -> None:
    """Create routes to test FastJWT dependencies."""
    ...
    async def _subjects_route():
        return {"subjects": [e for e in USER_DB.values()]}
