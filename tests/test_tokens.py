...
from fastjwt.token import create_token
from fastjwt.token import decode_token
...
def test_create_token_with_issuer():
    ...
    assert payload.get("iss") == "OCARINOW"
    payload = decode_token(
        token, key=KEY, algorithms=[ALGO], verify=True, issuer="OCARINOW"
    )
    assert payload.get("iss") == "OCARINOW"
...
def test_create_token_with_audience():
    ...
    payload = decode_token(
        token, key=KEY, algorithms=[ALGO], verify=True, audience=["OCARINOW"]
    )
    payload = decode_token(
        token, key=KEY, algorithms=[ALGO], verify=True, audience="OCARINOW"
    )
    payload = decode_token(
        token, key=KEY, algorithms=[ALGO], verify=True, audience=["FASTJWT"]
    )
    payload = decode_token(
        token, key=KEY, algorithms=[ALGO], verify=True, audience="FASTJWT"
    )
    payload = decode_token(
        token, key=KEY, algorithms=[ALGO], verify=True, audience=["OCARINOW", "FASTJWT"]
    )
    assert payload.get("aud") == ["OCARINOW", "FASTJWT"]
